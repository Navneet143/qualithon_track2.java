...
public class MoviePage extends Page {
	...
	public List<String> genres() {
		...
		for(WebElement we: this.testSession.driverWait().until(ExpectedConditions
				.presenceOfAllElementsLocatedBy(By.xpath("//div[@data-testid=\"genres\"]//a")))) {
			genres.add(we.getText());
		}
		if (genres.isEmpty()) {
	...
	public String releaseYear() {
		return this.testSession.driverWait()
				.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(
						"a[class='ipc-link ipc-link--baseAlt ipc-link--inherit-color sc-52284603-1 ifnKcw']")))
				.getText();
	}

	public String maturityRating() {
		return this.testSession.driverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(
				"//ul[@data-testid='hero-title-block__metadata']/li[2]/a[1]")))
				.getText();
	...
	public String score() {
		return this.testSession.driverWait()
				.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[@class='sc-7ab21ed2-1 jGRxWM']")))
				.getText();
